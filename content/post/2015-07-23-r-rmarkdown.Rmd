...
- What are their current attitudes towards the Liberal Party? Essentially, this questions provides the party with a pulse of how the riding is currently doing. The benefit of this question will help the party divert its efforts accordingly. If the riding holds a favourable view, then the party may better spend its efforts elsewhere. 
...
survey_results %>% 
  ...
  labs(title = "Figure 8: Rate the performance of the current federal government party?",
       ...
       tag = "[8]")
```
Figure 8 indicates a poor approval rating from the respondents. With an average score of 5, residents don't seem to be pleased with the current performance of the Justin Trudeau's Liberals.
```{r, echo=FALSE}
...
```
From figures 9 and 10, we gather that the residents of Richmond hill find some issues much more important than others. Primarily, they are concerned with unemployment, immigration, and affordable housing. They don't seem to be too concerned with issues such as Climate Change.
...
- The Liberal party has had close calls in their 2 recent elections for Richmond hill. Even in 2015, when the Liberals were able to reclaim GTA from the Conservatives, the election wasn't an easy victory.
...
![](img/important_issues.png)
```{r}
knitr::include_graphics('img/age_range.png')
knitr::include_graphics('img/2019_result.png')
```
